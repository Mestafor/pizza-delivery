...
cli.responders.help = () => {
  ...
  const commands = {
    ...
    'git log': 'Show the details of git user commit in last day'
  };
...
cli.responders.gitLog = (str) => {
  const log = childProcess.exec('git log --since=yesterday');
  log.stdout.on('data', buffer => {
