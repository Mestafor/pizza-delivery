...
server.processHandlerResponse = (res, method, trimedPath) => (statusCode, payload, contentType) => {
  ...
  if(statusCode === 200) {
    debug('\x1b[32m%s: %s %s\x1b[0m', method.toUpperCase(), trimedPath, statusCode);
  } else {
    debug('\x1b[31m%s: %s %s\x1b[0m', method.toUpperCase(), trimedPath, statusCode);
  }
}

/**
 * Unified server call for http and https protocol
 */
server.unifiedServer = (req, res) => {
  ...
  req.on('end', () => {
    ...
    try {
      choosenHandler(data, server.processHandlerResponse(res, method, trimedPath));
    } catch (e) {
